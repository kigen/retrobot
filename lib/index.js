         ...
         /* if (dnd.dnd_enabled) {
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, "👂 Hello!, It's time for our retrospective" +
                "\nLet's see what we did well, & what didn't do well"+
                '\nStart lines with `+` for "worked well" and `-` for "needs ' +
                "improvement."+
                "\n Example: "+
                "\n `+ We did well this time`"+
                "\n `- We did badly this time`"+
                "\n NB: Ensure you have space after the `+` or `-` symbols" 
                );
            });
          });
